...
#机械臂根据逆运动学算出的角度进行移动 the robot arm moves according to the angle calculated by inverse kinematics
ik = IK('arm')
#设置连杆长度 set connecting rod lenght
l1 = ik.l1
l4 = ik.l4
...
class ArmIK:
    ...
    def transformAngelAdaptArm(self, theta3, theta4, theta5, theta6):
        #将逆运动学算出的角度转换为舵机对应的脉宽值 covert the angle calculated by inverse kinematics into the pulse width value corresponding to the servo
        ...
        if servo3 > self.servo3Range[1] or servo3 < self.servo3Range[0]:
            logger.info('servo3(%s)超出范围 out of range (%s, %s)', servo3, self.servo3Range[0], self.servo3Range[1])
            return False
        ...
        if servo4 > self.servo4Range[1] or servo4 < self.servo4Range[0]:
            logger.info('servo4(%s)超出范围 our of range (%s, %s)', servo4, self.servo4Range[0], self.servo4Range[1])
            return False
        ...
        if servo5 > ((self.servo5Range[1] + self.servo5Range[0])/2 + 90*self.servo5Param) or servo5 < ((self.servo5Range[1] + self.servo5Range[0])/2 - 90*self.servo5Param):
            logger.info('servo5(%s)超出范围 out of range (%s, %s)', servo5, self.servo5Range[0], self.servo5Range[1])
            return False
        ...
        if servo6 > self.servo6Range[1] or servo6 < self.servo6Range[0]:
            logger.info('servo6(%s)超出范围 out of range (%s, %s)', servo6, self.servo6Range[0], self.servo6Range[1])
            return False
