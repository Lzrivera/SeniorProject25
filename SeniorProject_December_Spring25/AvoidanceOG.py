...
# 夹持器夹取时闭合的角度 the closing angle of the gripper when gripping
servo1 = 1500
...
# app停止玩法调用 app stops gameplay calling
def stop():
    global __isRunning
    __isRunning = False
    chassis.set_velocity(0,0,0)
...
# 设置避障速度 set obstacle avoidance speed
def setSpeed(args):
    global speed
    speed = int(args[0])
    return (True, ())
 
# 设置避障阈值    set obstacle avoidance threshold
def setThreshold(args):
    global Threshold
    Threshold = args[0]
    return (True, (Threshold,))

# 获取当前避障阈值 get the current obstacle avoidance threshold
...
def run(img):
    ...
    data_ = data.copy()
    u = data_.mean()  # 计算均值 calculate mean 1
    std = data_.std()  # 计算标准差 calculate stand dev
    ...
    if __isRunning:   
        if speed != old_speed:   # 同样的速度值只设置一次  the same speed value is set only once
            old_speed = speed
            chassis.set_velocity(speed,90,0)
            
        if distance <= Threshold:   # 检测是否达到距离阈值 check if the distance threshold is reached
            if turn:
                turn = False
                forward = True
                stopMotor = True
    ...
    return cv2.putText(img, "Dist:%.1fcm"%distance, (30, 480-30), cv2.FONT_HERSHEY_SIMPLEX, 1.2, TextColor, 2)  # 把超声波测距值打印在画面上 print the ultrasonic distance measurment value on the screen
